import {
  ...
  const fetcher: ResourceFetcher<true, GuestbookEntry[], GuestbookEntry | number> = async (
    _,
    { refetching, value },
  ) => {
    const isPost = refetching && typeof refetching !== "number";
    const method = isPost ? "POST" : "GET";
  
    const res = await fetch("/api/guestbook", {
      method,
      body: isPost ? JSON.stringify(refetching) : null,
    });
  ...
  export function Reviews({ reviews }: { reviews: GuestbookEntry[] }) {
    ...
    const onSubmitHandler: JSX.EventHandler<HTMLFormElement, SubmitEvent> = (
      ...
      refetch({ name, message, id: Date.now() });  // Aquí es correcto hacer un POST
      formElement.reset();
    };
    ...
    const handleDelete = async (id: number) => {
      ...
      refetch(); // Aquí aseguramos que se haga un GET después del DELETE
    };
            ...
            <For each={data()}>
              {(review) => (
                <li class="p-4 border rounded-md bg-white dark:bg-zinc-800 dark:border-zinc-700">
                  <a href="#" onClick={(e) => {e.preventDefault(); handleDelete(review.id);}}>X</a>
